...
/* $Id: sha.c,v 1.1.1.1 2003/07/07 07:33:44 ghost Exp $ */
...
void
HLSha1::sha_copy(struct sha_ctx *dest, struct sha_ctx *src)
{
  int i;
...
void
HLSha1::sha_init(struct sha_ctx *ctx)
{
...
void
HLSha1::sha_transform(struct sha_ctx *ctx, UInt32 *data )
{
...
void
HLSha1::sha_block(struct sha_ctx *ctx, UInt8 *block)
{
...
void
HLSha1::sha_update(struct sha_ctx *ctx, UInt8 *buffer, UInt32 len)
{
  if (ctx->index)
...
void
HLSha1::sha_digest(struct sha_ctx *ctx, UInt8 *s)
{
  int i;
...
void
HLSha1::sha_final(UInt8 *s, struct sha_ctx *ctx)
{
  UInt32 data[SHA_DATALEN];
  int i;
  int words;
...
int
HLSha1::sha_fd (int fd, size_t maxlen, UInt8 *md)
{
	struct sha_ctx ctx;
#define BLOCKSIZE 4096
	...
	for (;;) {
		...
		sha_update(&ctx, (unsigned char*)buffer, sum);
	}
...
void
HLSha1::HMAC_XXX (UInt8 *md, DataBuffer key, DataBuffer text)
{
	UInt8 mdkey[20];
	...
	if (key.len > 64)
		{
			...
			sha_init(&ctx);
			sha_update(&ctx, key.data, key.len);
			sha_final(mdkey, &ctx);
			key.len = 20;
			}
		key.data = mdkey;
	...
	for (i = 0; i < 64; i++)
		...
		sha_update(&ctx, text.data, text.len);
		sha_final(md, &ctx);

...
NewSndCallBackProc			   (SndCallBackProcPtr		userRoutine);
EXTERN_API(SndDoubleBackUPP)
NewSndDoubleBackProc		   (SndDoubleBackProcPtr	userRoutine);
EXTERN_API(SoundParamUPP)
NewSoundParamProc			   (SoundParamProcPtr		userRoutine);
EXTERN_API(SIInterruptUPP)
NewSIInterruptProc			   (SIInterruptProcPtr		userRoutine);
EXTERN_API(FilePlayCompletionUPP)
NewFilePlayCompletionProc	   (FilePlayCompletionProcPtr userRoutine);
EXTERN_API(SICompletionUPP)
NewSICompletionProc			   (SICompletionProcPtr		userRoutine);
EXTERN_API(void)
CallSndCallBackProc			   (SndCallBackUPP			userRoutine,
								SndChannelPtr			chan,
								SndCommand *			cmd);
EXTERN_API(void)
CallSndDoubleBackProc		   (SndDoubleBackUPP		userRoutine,
								SndChannelPtr			channel,
								SndDoubleBufferPtr		doubleBufferPtr);
EXTERN_API(Boolean)
CallSoundParamProc			   (SoundParamUPP			userRoutine,
								SoundParamBlockPtr *	pb);
EXTERN_API(void)
CallSIInterruptProc			   (SIInterruptUPP			userRoutine,
								SPBPtr					inParamPtr,
								Ptr						dataBuffer,
								short					peakAmplitude,
								long					sampleSize);
EXTERN_API(void)
CallFilePlayCompletionProc	   (FilePlayCompletionUPP	userRoutine,
								SndChannelPtr			chan);
EXTERN_API(void)
CallSICompletionProc		   (SICompletionUPP			userRoutine,
								SPBPtr					inParamPtr);
#else
...
#if MIXEDMODE_CALLS_ARE_FUNCTIONS
EXTERN_API(SICCompletionUPP)
NewSICCompletionProc		   (SICCompletionProcPtr	userRoutine);
EXTERN_API(void)
CallSICCompletionProc		   (SICCompletionUPP		userRoutine,

...
// c'est modifié plus bas si csv ou pas!
...
bool CMyApplication::DoOptions()
{
	...
	win.SetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mSerialNumber, mAdminLogin, 
				mAgreementBanner, mAgreementBannerUrl, mAgreementUrl, 
				&stPermBanList, mOptions);
#elif BANNER_SERVER
	win.SetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mAdminLogin, mAgreementBanner, 
	...
	win.SetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mSerialNumber, mAdminLogin, 
				&stPermBanList, mOptions);
#else
	win.SetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mAdminLogin, &stPermBanList, 
				mOptions);
#endif
	...
	for(;;)
	{
		...
		#if BANNER_SERVER & NETWORK_SERVER
			win.GetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mSerialNumber, mAdminLogin, mAgreementBanner, mAgreementBannerUrl, mAgreementUrl, &stPermBanList, stOptions);
		#elif BANNER_SERVER
			win.GetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mAdminLogin, mAgreementBanner, mAgreementBannerUrl, mAgreementUrl, &stPermBanList, stOptions);
		#elif NETWORK_SERVER
			win.GetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mSerialNumber, mAdminLogin, &stPermBanList, stOptions);
		#else
			win.GetInfo(mServerName, mIncorrectlogin, mMessageBeforedl, mPostBeforedl, mAcceptPrivate, mLoginMessageText, mServerDesc, mAdminLogin, &stPermBanList, stOptions);
		#endif
...
if (mOptions.nOpts & myOpt_kickunnamedusers)
	{
		...
		if (IsPermBannedName(inClient)){
			...
			data->AddPString(myField_Data, "\pYou are not allowed to use this name. Please change your name!");
			inClient->tpt->SendTransaction(myTran_ServerMsg, data);
			inClient->tpt->StartDisconnect();
...
void CMyApplication::ReadPrefs(SRect *outWinRects, Uint32 *outWinVis)
{
	...
	*outWinVis = unflat.ReadWord();
	...
	invalid:
		...
		mTrackerPassID = 0;
		mServerName[0] = mServerDesc[0] = mIncorrectlogin[0] = mMessageBeforedl[0] = mPostBeforedl[0] = mAcceptPrivate[0] = mLoginMessageText[0] = mAdminLogin[0] = 0;

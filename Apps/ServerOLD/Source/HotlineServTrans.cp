...
//------------------------------------------------------------------------------------------
	...
	data->DeleteAllFields();
	data->AddInteger(myField_Vers, 196);	// to let the client know that we're version 1.9.6
...
//bot
s = UText::Format(buf, sizeof(buf), "Nickname:   %#s\rName:       %#s\rAccount:    %#s\rAddress:    %#s\r\r-------- File Downloads ---------\r\r%s\r------- Folder Downloads --------\r\r%s\r--------- File Uploads ----------\r\r%s\r-------- Folder Uploads ---------\r\r%s\r------- Waiting Downloads -------\r\r%s\rUserId:     %#s\rIcon:       %#s\rAway:       %#s min %#s sec\rClient:     %#i\rDL Blocked: %#s\rInvisible : %#s\rTry to DL:  %#i x\rLogOn:      %#s\r", client->userName, client->accountName, client->accountLogin, ip, downFileText, downFolderText, upFileText, upFolderText, downWaitingText,userid, usericon, curmin, cursec, client->vers, blockeddownload, visible, client->dontChatCount, client->logon);
}else{
// NEW //
   s = UText::Format(buf, sizeof(buf), "Nickname:   %#s\rUserId:     %#s\rIcon:       %#s\rAway:       %#s min %#s sec\rClient:     %#i\rDL Blocked: %#s\rInvisible : %#s\rTry to DL:  %#i x\rName:       %#s\rAccount:    %#s\rAddress:    %#s\rLogOn:      %#s\r\r-------- File Downloads %#ix -----\r%s\r------- Folder Downloads %#ix -----\r%s\r--------- File Uploads %#ix ------\r%s\r-------- Folder Uploads %#ix -----\r%s\r------- Waiting Downloads -------\r%s", client->userName, userid, usericon, curmin, cursec, client->vers, blockeddownload, visible, client->dontChatCount, client->accountName, client->accountLogin, ip, client->logon, client->numdl, downFileText,client->numflddl, downFolderText, client->numul, upFileText,client->numfldul, upFolderText, downWaitingText);
}

...
CContainerView *CMyOptionsWin::MakeGeneralTab()
{
	...
	{
		// make box
		box = new CBoxView(vc, SRect(10,10,540,136), boxStyle_Etched);
		box->Show();
		...
		// make box
		box = new CBoxView(vc, SRect(10,146,540,242), boxStyle_Etched);
		box->Show();
...
CContainerView *CMyOptionsWin::MakeMiscTab()
{
	...
	try
	{
		...
		//Kick Unnamed Users
		chk = new CCheckBoxView(vc, SRect(20,62,290,120));
		chk->SetTitle("\pKick Banned Users Names listed on Ban tab");
		chk->SetAutoMark(true);
		chk->Show();
...
void CMyOptionsWin::SetInfo(const Uint8 *inName, const Uint8 *inIncorrectlogin, const Uint8 *inMessagebeforedl, const Uint8 *inPostbeforedl, const Uint8 *inAcceptPrivate, const Uint8 *inLoginMessageText, const Uint8 *inDesc, 
...
void CMyOptionsWin::SetInfo(const Uint8 *inName, const Uint8 *inIncorrectlogin, const Uint8 *inMessagebeforedl, const Uint8 *inPostbeforedl, const Uint8 *inAcceptPrivate, const Uint8 *inLoginMessageText, const Uint8 *inDesc, 
							...
							const SMyOptions& inOptions)
#elif NETWORK_SERVER
void CMyOptionsWin::SetInfo(const Uint8 *inName, const Uint8 *inIncorrectlogin, const Uint8 *inMessagebeforedl, const Uint8 *inPostbeforedl, const Uint8 *inAcceptPrivate, const Uint8 *inRefusePrivate, const Uint8 *inLoginMessageText, const Uint8 *inDesc, 
							Uint8* inSerialNumber, const Uint8 *inAdminLogin, 
							CPtrList<BanRecord> *inPermBanList, 
							const SMyOptions& inOptions)
#else
...
#if BANNER_SERVER & NETWORK_SERVER
void CMyOptionsWin::GetInfo(Uint8 *outName, Uint8 *outIncorrectlogin, Uint8 *outMessagebeforedl, Uint8 *outPostbeforedl, Uint8 *outAcceptPrivate, Uint8 *outLoginMessageText, Uint8 *outDesc, 
...
#endif
{
	...
	if (outIncorrectlogin) 
		outIncorrectlogin[0] = mViews.incorrectlogin->GetText(outIncorrectlogin + 1, 127);
	
	// ChatBefore DL
	if (outMessagebeforedl) 
		outMessagebeforedl[0] = mViews.messagebeforedl->GetText(outMessagebeforedl + 1, 127);
	
	// PostBefore DL
	if (outPostbeforedl) 
		outPostbeforedl[0] = mViews.postbeforedl->GetText(outPostbeforedl + 1, 127);
		
	// RefusePrivate
	if (outAcceptPrivate) 
		outAcceptPrivate[0] = mViews.acceptprivate->GetText(outAcceptPrivate + 1, 127);
	
	// LoginMessageText
	if (outLoginMessageText) 
		outLoginMessageText[0] = mViews.loginmessagetext->GetText(outLoginMessageText + 1, 127);
